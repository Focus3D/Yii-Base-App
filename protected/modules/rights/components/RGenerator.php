...
class RGenerator extends CApplicationComponent
{
	 ...
	 * @return array the controllers.
	 */
	protected function getControllersInPath($path, $parentFolder = '')
	{
		$controllers = array();
			...
			foreach( $controllerDirectory as $entry )
			{
				if( $entry{0}!=='.' )
				{
					...
					if( strpos(strtolower($entry), 'controller')!==false )
					{
						...
						$fullName = str_replace(DIRECTORY_SEPARATOR, '.', $parentFolder . $name);

						$controllers[ strtolower($fullName) ] = array(
                                                        'name'=> $fullName,
                                                        'file'=>$entry,
                                                        'path'=>$entryPath,
						);
					...
					if( is_dir($entryPath)===true ) {
						$folder = (empty($parentFolder)) ? $entry : $parentFolder.$entry;
						foreach( $this->getControllersInPath($entryPath, $folder.DIRECTORY_SEPARATOR) as $controllerName=>$controller ) {
